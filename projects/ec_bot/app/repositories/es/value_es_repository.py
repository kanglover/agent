...
class ValueESRepository:
    ...
    async def index(self, value_infos: list[ValueInfo], batch_size=20):
        ...
        for batch_value_infos in batched(value_infos, batch_size):
            batch_operations = [
                item
                for value in batch_value_infos
                for item in ({"index": {"_index": self.index_name, "_id": value.id}}, asdict(value))
            ]
