...
class GenerationIntegrationModule:
    """
    Generation Integration Module.
    """

    def __init__(
        self, model_name: str = "", temperature: float = 0.1, max_tokens: int = 2048
    ):
    ...
    def generate_basic_answer(self, query: str, context_docs: list[Document]) -> str:
        """
        Generate a basic answer using the LLM model.
        """
    ...
    def generate_step_by_step_answer(
        ...
        """
        Generate a step-by-step answer using the LLM model.
        """
        context = self._build_context(context_docs)

        prompt = ChatPromptTemplate.from_template(
            """
            你是一位专业的烹饪助手。请根据以下食谱信息，为用户提供详细的分步指导。
            用户问题: {question}
    ...
    def query_rewrite(self, query: str) -> str:
        ...
        if response != query:
            logger.info(f"Query rewritten from '{query}' to '{response}'")
        else:
            logger.info(f"Query '{query}' does not require rewriting")

        return response

    def query_router(self, query: str) -> str:
        """
        Route the query to the appropriate processing chain based on its content.
        """
        prompt = ChatPromptTemplate.from_template(
            """
    ...
    def generate_list_answer(self, query: str, context_docs: list[Document]) -> str:
        """
        Generate a list answer using the LLM model.
        """
        if not context_docs:
            return "暂无相关食谱信息，无法提供菜品列表或推荐。"
    ...
    def generate_step_by_step_answer_stream(self, query: str, context_docs: list[Document]):
        ...
        prompt = ChatPromptTemplate.from_template(
            """
            你是一位专业的烹饪助手。请根据食谱信息，为用户提供详细的分步骤指导。
            用户问题: {question}
    ...
    def _build_context(
        self, context_docs: list[Document], max_length: int = 2000
    ) -> str:
        """
        Build a context string from the provided documents.
        """
        if not context_docs:
            return "暂无相关食谱信息"
        ...
        for i, doc in enumerate(context_docs, 1):
            ...
            if "dish_name" in doc.metadata:
                metadata_info += f"菜名: {doc.metadata['dish_name']}; "
